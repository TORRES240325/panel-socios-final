...
from db_models import Usuario, Producto, Key, inicializar_db, get_session # Importamos la nueva tabla Key

# Inicializa la base de datos (asegura que las tablas existan)
inicializar_db()
...
TOKEN = '8570947665:AAFUqtimxOzs-KPhqavl53N9RRxi21bb4RM' 
...
async def show_buy_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    with get_session() as session_db:
        ...
        # Obtiene todas las categorías dinámicamente de la DB
...
async def handle_category_selection(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    with get_session() as session_db:
        # Buscamos todos los productos dentro de la categoría seleccionada
    ...
    for producto in productos:
        # Calcular stock disponible usando la tabla Key
        with get_session() as s:
            stock = s.query(Key).filter(Key.producto_id == producto.id, Key.estado == 'available').count()
        
        # El formato del botón debe ser [Key Fluorite (1 Day)] o [Fluorite]
...
async def handle_final_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Procesa las selecciones de compra (files/status/buy)."""
    text = update.message.text
    user_id_telegram = update.effective_user.id
    ...
    # --- Lógica de Acciones (Get files / Check status) ---
    if text in ["Get files", "Check status"]:
        # Implementa aquí la lógica para obtener archivos o chequear el estado de la clave
        await update.message.reply_text(f"Action '{text}' is not yet implemented.", reply_markup=update.message.reply_markup)
        return BUY_PRODUCT
    
    # --- Lógica de Compra (Buy) de un producto específico ---
    session_db = get_session()
    try:
        # Intentamos parsear el texto del botón para obtener el nombre y precio
        parts = text.rsplit(' - $', 1) 
        if len(parts) != 2:
            raise ValueError("Invalid product format.")
            
        product_name = parts[0].strip()
        price_str = parts[1].split('(')[0].strip() # Quitamos el stock para obtener el precio
        ...
        available_key = session_db.query(Key).filter_by(
            ...
            estado='available'
        ).with_for_update().first() # Bloqueamos la clave para evitar doble venta
        ...
        await update.message.reply_text(
            ...
            f"🔐 **Tu Key/Licencia:** `{available_key.licencia}`", # Entrega la clave real
            parse_mode='Markdown'
        )
